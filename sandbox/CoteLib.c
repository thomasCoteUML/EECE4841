...
imgTC pixelProcTC(imgTC image, unsigned char (*func)(int, int, unsigned char, int*) ) {
	...
	for(int i=0; i<image.sizeX; i++){
		...
		for(int j=0; j<image.sizeY; j++){
			...
			/* 
				...
				allow unknown function to control indexing of k
			*/
			tmp = (*func)(i, j, processMatrix[i][j], &k);
			if (tmp != 0x00) {
